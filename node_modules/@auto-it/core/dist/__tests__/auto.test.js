...
describe("Auto", () => {
    ...
    test("should extend config and keep overrides", async () => {
        search.mockReturnValueOnce({ config: Object.assign(Object.assign({}, defaults), { extends: "@artsy", name: 'extended' }) });
    ...
    test("should extend local config and keep overrides", async () => {
        ...
        process.cwd = () => "/foo/";
        search.mockReturnValueOnce({
            config: Object.assign(Object.assign({}, defaults), { extends: "./fake.json", name: 'extended' }),
        });
        importMock.mockImplementation((path) => path === "/foo/fake.json" ? { noVersionPrefix: true, name: 'fake' } : undefined);
        const auto = new auto_1.Auto();
        auto.logger = logger_1.dummyLog();
        await auto.loadConfig();
        expect(auto.release.config).toMatchSnapshot();
    ...
    describe("release", () => {
        ...
        test("should publish with default options", async () => {
            ...
            expect(auto.git.publish).toHaveBeenCalledWith("releaseNotes", "v1.2.4", false, "", true);
        ...
        test("should use --to commit target", async () => {
            ...
            expect(auto.git.publish).toHaveBeenCalledWith("releaseNotes", "v1.2.4", false, "abc", true);
        });
        ...
        test("should a prerelease", async () => {
            ...
            expect(auto.release.generateReleaseNotes).toHaveBeenCalledWith("v1.2.3", undefined, undefined);
            expect(auto.git.publish).toHaveBeenCalledWith("releaseNotes", "v1.2.4", true, "", false);
            expect(afterRelease).toHaveBeenCalledWith(expect.objectContaining({
                lastRelease: "v1.2.3",
                newVersion: "v1.2.4",
            }));
        ...
        test("should publish with newVersion using useVersion option", async () => {
            ...
            expect(auto.release.generateReleaseNotes).toHaveBeenCalledWith("v1.2.3", undefined, undefined);
            expect(auto.git.publish).toHaveBeenCalledWith("releaseNotes", "v1.3.0", false, "", true);
            expect(afterRelease).toHaveBeenCalledWith(expect.objectContaining({
                lastRelease: "v1.2.3",
                newVersion: "v1.3.0",
            }));
        ...
        test("should publish with newVersion using useVersion option with semver build part", async () => {
            ...
            expect(auto.release.generateReleaseNotes).toHaveBeenCalledWith("v1.2.3", undefined, undefined);
            expect(auto.git.publish).toHaveBeenCalledWith("releaseNotes", "v1.2.3+1", false, "", true);
            expect(afterRelease).toHaveBeenCalledWith(expect.objectContaining({
                lastRelease: "v1.2.3",
                newVersion: "v1.2.3+1",
            }));
